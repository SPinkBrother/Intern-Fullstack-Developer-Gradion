...
export function RegisterPage({ initialEmail = "", onRegister, onLogin }: { initialEmail?: string; onRegister: (user: User) => void; onLogin: (email: string) => void }) {
  ...
  function handleLoginClick() { onLogin(email); }
  ...
  return (
    <AuthLayout>
      <form className="w-full max-w-[430px] rounded-2xl border border-line/80 bg-surface p-6 shadow-card sm:p-9" onSubmit={handleSubmit} noValidate>
        ...
        <button className="mt-5 w-full text-sm font-bold text-brand underline-offset-4 hover:text-brand-hover hover:underline" type="button" onClick={handleLoginClick}>Already have an account? Sign in</button>
      </form>
    </AuthLayout>
  );
}
